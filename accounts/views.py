...
def auth_page(request):
    """Combined login and registration page with split-screen design."""
    ...
    if request.method == "POST":
        ...
        if form_type == "login":
            ...
            if limited:
                login_errors.append(f"Too many login attempts. Please wait {retry_after} seconds.")
            else:
                ...
                if login_form.is_valid():
                    ...
                    if user.is_active:
                        ...
                        login_errors.append("Your account is inactive. Please contact support.")
                else:
                    for field, errors in login_form.errors.items():
                        for error in errors:
                            login_errors.append(f"{error}")

        elif form_type == "signup":
            # Handle Registration
            limited, retry_after = rate_limit_request(request, "register", limit=4, window=300)
            if limited:
                signup_errors.append(f"Please wait {retry_after} seconds before creating another account.")
            else:
                ...
                if signup_form.is_valid():
                    try:
                        ...
                        signup_errors.append("An error occurred while creating your account. Please try again.")
                else:
                    for field, errors in signup_form.errors.items():
                        for error in errors:
                            signup_errors.append(f"{error}")
    ...
    return render(
        ...
        "accounts/auth.html",
        {
            ...
            **build_seo_context(
                request,
                title=f"Login & Register | {settings.SITE_NAME}",
                description=f"Sign in to your {settings.SITE_NAME} account or create a new account to access premium content.",
